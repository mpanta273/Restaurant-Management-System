...
<?php
            ...
            $id_quantity = $_POST['quantity'];
                ...
                if (is_numeric($id_refnumber) and is_numeric($id_quantity) and ($id_quantity <= 5 and $id_quantity >= 1) and ($id_refnumber > 0)){
                    ...
                    if($result_select_cart->num_rows > 0){
                        $sql_cart = "UPDATE Cart SET Quantity = $id_quantity WHERE CartID = $id_refnumber;";
                        $result_cart = $conn->query($sql_cart);
                            echo "Updated cart item successfully";
                            echo "<br>"; 
                    }else{
                        echo "A record with the given reference number was not found.";
                    }
                }else{
                    echo "Error. Invalid Quantity or Reference Number.";
                }
?>
